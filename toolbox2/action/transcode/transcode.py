...
class TranscodeAction(Action):
    ...
    def __init__(self, log, base_dir, _id, params=None, resources=None):
        ...
        self.audio_channels_per_stream = int(
            self.params.get("audio_channels_per_stream", 0)
        )
        ...
        self.container_essence_dir = self.params.get(
            "container_essence_dir", "media.dir/"
        ).lstrip("/")
        self.container_abs_essence_dir = os.path.join(
            self.tmp_dir, self.container_essence_dir
        )
        ...
        if self.muxer == "ffmpeg" and self.container_reference:
            raise TranscodeException(
                "Reference files are not supported with ffmpeg muxer"
            )
            ...
            if not self.container_hinting:
                ...
                if self.container in ["flv"]:
                    ...
                    for index, output_file in enumerate(ffmpeg.output_files):
                        ...
                        self.workers.append(hinting_worker)
                        self.add_output_resource(
                            index + 1, {"path": hinting_output_path}
                        )
