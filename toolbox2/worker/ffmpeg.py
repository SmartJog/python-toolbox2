...
class FFmpegWorker(Worker):
            ...
            for option in all_opts:
                if isinstance(option, list) or isinstance(option, tuple):
                    ...
                    raise FFmpegWorkerException(
                        "FFmpeg options must be of type tuple or list"
                    )
    ...
    def set_timecode(self, timecode):
        ...
        self.format_opts += [("-timecode", timecode)]
    ...
    def set_aspect_ratio(self, aspect_ratio):
        ...
        self.video_opts += [("-aspect", aspect_ratio)]
    ...
    def _get_input_avinfo(self, index=0):
        ...
        if not avinfo:
            raise FFmpegWorkerException(
                "No AVInfo specified for input file: %s" % self.input_files[index].path
            )
    ...
    def _get_audio_layout_mapping(self):
        ...
        for audio_stream in avinfo.audio_streams:
            ...
            if self.channels_per_stream > 0:
                channels_left = self.channels_per_stream - len(
                    o_stream_map[o_stream_idx]["input_channels"]
                )
            else:
                channels_left = channels
            if channels == channels_left:
                o_stream_map[o_stream_idx]["input_streams"][
                    audio_stream["index"]
                ] = audio_stream
                o_stream_map[o_stream_idx]["input_channels"].append(
                ...
                for channel_idx in range(channels):
                    ...
                    o_stream_map[o_stream_idx]["input_channels"].append(
                        (audio_stream["index"], channel_idx)
                    )
                    if (
                        channels > channels_left
                        and len(o_stream_map[o_stream_idx]["input_channels"])
        ...
        for index, output_stream in list(o_stream_map.items()):
            if len(output_stream["input_streams"]) == 1:
                ...
                if (
                    input_stream["channels"] == self.channels_per_stream
                    or self.channels_per_stream == 0
                ):
                    map_chain.append(("-map", "0:%s" % input_stream["index"]))
                else:
                    filter_merge = ""
                    for input_channel in output_stream["input_channels"]:
                        filter_chain += "[0:%s]pan=mono:c0=c%s" % (
                            input_channel[0],
                            input_channel[1],
                        )
                        filter_chain += "[p%s_%s];" % (
                    ...
                    if len(output_stream["input_channels"]) > 1:
                        ...
                        filter_chain += "amerge=inputs=%s[m%s];" % (
                            len(output_stream["input_channels"]),
                            index,
                        )
                        ...
                        map_chain.append(
                            (
                                ...
                                "[p%s_%s]"
                                % (
                                    output_stream["input_channels"][0][0],
                                    output_stream["input_channels"][0][1],
                                ),
                            )
                        )
            else:
        ...
        if self.audio_min_streams and not self.nb_frames:
            raise FFmpegWorkerException(
                "audio_min_streams option requires input file nb frames to be computed"
            )
        else:
            ...
            for index in range(empty_streams):
                filter_chain += "aevalsrc=%s:n=480:s=48000:d=%s[null%s];" % (
                    "0:" * i_channels_per_stream,
                    duration,
                    index,
                )
    ...
    def make_thumbnail(self, options=None):
        ...
        if "duration" in avinfo.video_streams[0]:
            frame_count = int(
                avinfo.video_fps * float(avinfo.video_streams[0]["duration"])
            )
    ...
    def demux(self, basedir):
        ...
        for audio_map in mapping:
            ...
            extension = self._get_codec_extension(
                audio_codec or avinfo.video_streams[0]["codec_name"]
            )
            path = "%s%s" % (path, extension)
    ...
    def get_dnxhd_profile(self, profile):
        ...
        for dnxhd_profile in dnxhd_profiles:
            if (
                profile["bitrate"] == dnxhd_profile["bitrate"]
                and profile["pix_fmt"] == dnxhd_profile["pix_fmt"]
                and profile["interlaced"] == dnxhd_profile["interlaced"]
                and abs(profile["fps"] - dnxhd_profile["fps"]) < 0.1
            ):
    ...
    def transcode_dnxhd(self, options=None):
        ...
        if not avinfo.video_is_HD():
            raise FFmpegWorkerException(
                "DNxHD transcode does not support input at %s, only 1920x1080 is supported"
                % avinfo.video_res
            )
        ...
        if not dnxhd_profile:
            raise FFmpegWorkerException(
                "DNxHD output settings are not valid: %sMbps 1920x1080%s %sbit @ %sfps"
                % (
                    bitrate_m,
                    "i" if interlaced else "p",
                    10 if pix_fmt == "yuv422p10le" else 8,
                    avinfo.video_fps,
                )
            )
    ...
    def transcode_dv(self, options=None):
        ...
        if not avinfo.video_is_SD_PAL() and not avinfo.video_is_SD_NTSC():
            raise FFmpegWorkerException(
                "DV codec only supports PAL and NTSC resolutions"
            )

        self.video_opts = [("-vcodec", "dvvideo"), ("-pix_fmt", pix_fmt)]
        ...
        if avinfo.video_has_vbi:
            if avinfo.video_is_SD_NTSC():
                ...
                height = 576
            self.video_filter_chain.insert(
                0, ("crop_vbi", "crop=720:%s:00:32" % height)
            )
    ...
    def transcode_simple_h264(self, options=None):
        ...
        if resolution != "default":
            self.video_filter_chain.insert(1, ("scale", "scale=%s" % resolution))
    ...
    def transcode_mpeg2video(self, options=None):
        ...
        if pix_fmt == "yuv422p":
            ...
            raise FFmpegWorkerException(
                "MPEG-2 video codec does not support %s pixel format" % pix_fmt
            )
    ...
    def transcode_xdcamhd(self, options=None):
        ...
        if avinfo.video_res != avinfo.RES_HD:
            raise FFmpegWorkerException(
                "XDCAMHD transcode does not support input at %s, only 1920x1080 is supported"
                % avinfo.video_res
            )

        if bitrate not in [50000]:
            raise FFmpegWorkerException(
                "XDCAMHD does not support bitrate at %sk" % bitrate
            )
        ...
        if avinfo.video_fps in avinfo.FPS_NTSC:
            ...
            raise FFmpegWorkerException(
                "XDCAMHD does not support input at %s fps" % avinfo.video_fps
            )
    ...
    def mux_mpegps(self, basedir, options=None):
        ...
        if acodec != "mp2":
            raise FFmpegWorkerException(
                "MPEG-2 PS does not support audio codec: %s" % acodec
            )

        if vcodec not in ["mpeg1video", "mpeg2video"]:
            raise FFmpegWorkerException(
                "MPEG-2 PS does not support video codec: %s" % vcodec
            )
    ...
    def letterbox(self):
        ...
        if not avinfo.video_is_SD_NTSC() and not avinfo.video_is_SD_PAL():
            raise FFmpegWorkerException(
                "SD Letterboxing only supports input at PAL or NTSC resolutions"
            )

        self.set_aspect_ratio("4:3")

        if not avinfo.video_has_vbi or (
            avinfo.video_has_vbi and not self.keep_vbi_lines
        ):
            # Remove previously insered vbi lines and re-add them at the end
            # of the filter chain
            self.video_filter_chain = [
                flt for flt in self.video_filter_chain if flt[0] not in ["add_vbi"]
            ]
            ...
            self.video_filter_chain.append(
                (
                    "letterbox",
                    "scale=720:%d,pad=720:%d:00:%d"
                    % (scale_height, total_height, top_blank_lines),
                ),
            )
    ...
    def drawtext(self, basedir, options=None):
        ...
        fontfiles = {
            ...
            "liberation": toolbox2.get_internal_resource(
        ...
        self.video_filter_chain.append(("drawtext", drawtext_filter))
